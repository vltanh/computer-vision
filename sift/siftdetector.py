...
def detect_keypoints(imagename, threshold):
    ...
    kvec = np.array([[1.6 * k ** j for j in range(i, i + size_of_octaves)] for i in range(num_of_octaves)])
    kvectotal = np.array([1.6 * k ** i for i in range(2*size_of_octaves)])
    ...
    # Downsampling images
    octave = [misc.imresize(original, 200, 'bilinear').astype(int)]
    for i in range(num_of_octaves):
        octave.append(misc.imresize(octave[i], 50, 'bilinear').astype(int))
    ...
    pyrlvl = np.array([np.zeros((octave[i].shape[0], octave[i].shape[1], size_of_octaves)) for i in range(num_of_octaves)])
    ...
    # In each of the following for loops, elements of each pyramids that are larger or smaller 
    # than its 26 immediate neighbors in space and scale are labeled as extrema. As explained 
    # in section 4 of Lowe's paper, these initial extrema are pruned by checking that their contrast 
    # and curvature are above certain thresholds. The thresholds used here are those suggested by Lowe. 
    for i in range(1, 4):
