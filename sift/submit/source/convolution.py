...
def filter(img, kernel):
    ...
    # Apply kernel
    filtered = np.zeros((img_h, img_w)).astype('float')
    for r in range(padding_v, img_h - padding_v):
        for c in range(padding_h, img_w - padding_h):
            filtered[r, c] = np.sum(img[r - padding_v: r + padding_v + 1, c - padding_h: c + padding_h + 1] * kernel)
    return filtered
...
def Laplacian(x,y,sigma):
    return - 1.0 / (np.pi * sigma**4) * (1 - (x**2 + y**2) * 1.0 / (2 * sigma**2)) * np.exp(-(x**2 + y**2) * 1.0 / (2 * sigma**2))

def get_laplacian_kernel(size, sigma):
    kernel = np.zeros((size, size))
    radius = size // 2
    for i in range(-radius, radius + 1):
        for j in range(-radius, radius + 1):
            kernel[i + radius, j + radius] = Laplacian(i, j, sigma)
    print(np.sum(kernel))
    return kernel
